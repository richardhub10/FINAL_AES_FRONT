...
function getPhtPartsFromDate(d) {
  try {
    if (!(d instanceof Date) || Number.isNaN(d.getTime())) return { ymd: '', hhmm: '' };
    ...
    // Fallback: apply a fixed +8h offset.
    const pht = new Date(d.getTime() + 8 * 60 * 60 * 1000);
    return {
      ymd: pht.toISOString().slice(0, 10),
      hhmm: pht.toISOString().slice(11, 16),
    };
  } catch (e) {
    return { ymd: '', hhmm: '' };
  }
...
function getPhtPartsFromIso(iso) {
  if (!iso || typeof iso !== 'string') return { ymd: '', hhmm: '' };
  const d = new Date(iso);
  if (Number.isNaN(d.getTime())) return { ymd: '', hhmm: '' };
  return getPhtPartsFromDate(d);
}

function phtNowYmd() {
  return getPhtPartsFromDate(new Date()).ymd || new Date().toISOString().slice(0, 10);
}

function phtYmdHhmmToUtcIso(ymd, hhmm) {
  // Interpret inputs as a wall-clock time in PHT, then convert to a UTC ISO string.
  // Example: 2026-04-14 07:00 PHT -> 2026-04-13T23:00:00.000Z
  ...
  const utcMs = Date.UTC(y, m - 1, day, hh - 8, mm, 0, 0);
  const dUtc = new Date(utcMs);
  return Number.isNaN(dUtc.getTime()) ? '' : dUtc.toISOString();
}

function formatIsoForSticker(iso) {
  if (!iso || typeof iso !== 'string') return '—';

  // Ticket should show Philippines time (PHT, UTC+8).
  // Prefer Intl with an explicit timezone so it's correct even if device TZ differs.
  try {
    const parts = getPhtPartsFromIso(iso);
    if (parts.ymd && parts.hhmm) return `${parts.ymd} ${parts.hhmm} PHT`;
    return iso;
  } catch (e) {
    return iso;
  }
...
function downloadAppointmentStickerPdf({
  ...
  doc.text('All times are shown in PHT (UTC+8).', boxX, boxY + 66 + boxH + 44);
  ...
  const safeDate = getPhtPartsFromIso(scheduledForIso).ymd || 'date';
...
export default function App() {
  ...
  const [staffInboxSort, setStaffInboxSort] = useState('latest');

  const scheduledForIso = useMemo(() => {
    if (!selectedDateYmd || !selectedTime) return '';
    // User selects clinic local time (PHT). Convert to a UTC ISO string for the API.
    return phtYmdHhmmToUtcIso(selectedDateYmd, selectedTime);
  ...
  const bookedCountByDate = useMemo(() => {
    ...
    for (const appt of appointments || []) {
      ...
      const ymd = getPhtPartsFromIso(iso).ymd;
      if (!ymd) continue;
  ...
  const staffInboxAppointmentsSorted = useMemo(() => {
    const items = [...(staffInboxAppointments || [])];
    const dir = staffInboxSort === 'oldest' ? 1 : -1; // latest first by default
    items.sort((a, b) => {
    ...
    return items;
  }, [staffInboxAppointments, staffInboxSort]);
  ...
  const staffAppointmentsForSelectedDate = useMemo(() => {
    ...
    items.sort((a, b) => {
      ...
      const bIso = typeof b?.scheduled_for === 'string' ? b.scheduled_for : '';
      const aKey = `${getPhtPartsFromIso(aIso).ymd} ${getPhtPartsFromIso(aIso).hhmm}`;
      const bKey = `${getPhtPartsFromIso(bIso).ymd} ${getPhtPartsFromIso(bIso).hhmm}`;
  ...
  function slotsLeftForIso(iso) {
    ...
    const ymd = parts.ymd;
    const hhmm = parts.hhmm;
    if (!ymd || !hhmm) return HOURLY_CAPACITY;
    const key = `${ymd} ${hhmm}`;
    const used = confirmedCountByYmdHourGlobal.get(key) || 0;
    return Math.max(0, HOURLY_CAPACITY - used);
  }
  ...
  const earliestAvailableYmd = useMemo(() => {
    const start = new Date(`${phtNowYmd()}T00:00:00Z`);
    for (let i = 0; i < 366; i++) {
                  ...
                  <View style={[styles.actionRow, { marginBottom: 8 }]}>
                    <View style={styles.actionBtn}>
                      <UiButton
                        ...
                        onPress={() => setStaffInboxSort('latest')}
                        disabled={busy}
                        variant={staffInboxSort === 'latest' ? 'primary' : 'secondary'}
                      />
                    </View>
                    <View style={styles.actionBtn}>
                      <UiButton
                        title="Oldest requests"
                        onPress={() => setStaffInboxSort('oldest')}
                        disabled={busy}
                        variant={staffInboxSort === 'oldest' ? 'primary' : 'secondary'}
                      />
                    </View>
                  </View>
                  <FlatList
                    data={staffInboxAppointmentsSorted}
                    scrollEnabled={false}
                    keyExtractor={(item) => String(item.id)}
                    ...
                    renderItem={({ item }) => (
                      <View style={styles.item}>
                        ...
                        <Text style={styles.itemMeta}>{formatIsoForSticker(item.scheduled_for)}</Text>
